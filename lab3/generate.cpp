...
int main()
{
    gen_double_sinus_in_file(3,10,"si");
    gen_double_random_in_file(1,10,"ra");
    gen_double_sorted_in_file(2049,-4,0, "so");
...
void gen_double_sinus_in_file(size_t size, double start, const char * file_name )
{
    ...
    if (dst)
    {
        ...
        lab1<double>::sinusoidal_seq(arr,size, 10, 20,M_PI_2, start);
...
void gen_double_sorted_in_file(size_t size, double start, bool is_rev, const char * file_name )
{
    ...
    {
        double *arr = new double[size];
        lab1<double>::sorted(arr,size, start,2, is_rev);
        for (size_t i = 0; i < size; i++)
        {
            fprintf(dst, "%lf\n", arr[i]);
        }
